...
@section('title')
    Cedar - Purchase History
@endsection

@php
    $id = Auth::user()->id;
    $purchaseHistory = App\Models\Payment::where('user_id', $id)->latest()->get();
@endphp
            ...
            @foreach ($purchaseHistory as $key => $item)
                @if ($item->course)
                    <tr>
                        ...
                        <td><img src="{{ asset($item->course->course_image) }}" class="product-img-2" alt="product img"
